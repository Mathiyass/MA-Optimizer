...
export function StatusBar() {
    ...
    const cpuColor = cpu > 60 ? '#FF003C' : '#00FFDE'
    const ramColor = ram.percent > 60 ? '#FF003C' : '#00FFDE'
    ...
    return (
        <footer className="h-8 bg-[rgba(15,17,26,0.6)] backdrop-blur-3xl border-t border-white/5 flex items-center px-4 shrink-0 select-none text-[11px] z-20">
            ...
            <button
                ...
                className={`flex items-center gap-1.5 px-2 py-0.5 rounded-2xl transition-all ${logOpen ? 'text-accent-cyan bg-accent-cyan/10' : 'text-text-dim hover:text-text-primary'
                    }`}
            >
                <Terminal className="w-3 h-3" />
                <span>Console</span>
                <kbd className="ml-1 px-1 bg-card-bg border border-card-border rounded-2xl text-[9px]">Ctrl+L</kbd>
            </button>
        </footer>
    )
}
