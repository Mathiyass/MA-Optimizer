...
import { Cpu, Zap, ShieldAlert, MemoryStick, RefreshCw, Check, Loader2, Layers, Sliders, Ban, Activity, Sparkles } from 'lucide-react'
...
export function ProcessLassoPage() {
    ...
    return (
        <div className="space-y-8 max-w-[90rem] mx-auto w-full pb-10">
            ...
            {/* AI Process Governor Intelligence Card */}
            <div className="bg-[rgba(255,255,255,0.03)] backdrop-blur-3xl border border-[var(--accent-cyan)]/25 rounded-[2rem] p-6 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <div className="flex-1 space-y-1.5 relative z-10 text-left">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--accent-cyan)]">AI Governor Intelligence</span>
                        <span className="text-[9px] font-black uppercase px-2 py-0.5 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-400">
                            ProBalance Heuristics Active
                        </span>
                    </div>
                    <h3 className="text-white text-base font-black tracking-wide">
                        Real-Time CPU Contention & Thread Priority Governor
                    </h3>
                    <p className="text-[var(--text-secondary)] text-xs font-medium leading-relaxed max-w-2xl">
                        ProBalance monitors background threads every 5s. When any unprioritized process exceeds {config.proBalanceCpuThreshold}% CPU utilization, the governor demotes its priority to <span className="text-[var(--accent-cyan)] font-mono">BelowNormal</span> to preserve smooth 0-latency input and 1% low frame-rates for foreground gaming.
                    </p>
                </div>
