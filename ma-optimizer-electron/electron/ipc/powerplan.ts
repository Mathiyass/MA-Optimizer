...
async function run(cmd: string, args: string): Promise<string> {
    return new Promise(resolve => {
        exec(`${cmd} ${args}`, { timeout: 30000, windowsHide: true }, (err, stdout) => {
...
function reg(regPath: string, name: string, value: string | number, type = 'REG_DWORD') {
    try {
        execSync(`reg add "${regPath}" /v "${name}" /t ${type} /d ${value} /f`, { timeout: 10000, windowsHide: true })
    } catch { }
}
...
async function createMaPowerPlan(): Promise<boolean> {
    try {
        ...
        for (const [guid, ac, dc] of procTweaks) {
            await run('powercfg', `/setacvalueindex ${MA_PLAN_GUID} ${SUB_PROC} ${guid} ${ac}`)
            await run('powercfg', `/setdcvalueindex ${MA_PLAN_GUID} ${SUB_PROC} ${guid} ${dc}`)
        }
        ...
        for (const [guid, val] of sleepTweaks) {
            await run('powercfg', `/setacvalueindex ${MA_PLAN_GUID} ${SUB_SLEEP} ${guid} ${val}`)
            await run('powercfg', `/setdcvalueindex ${MA_PLAN_GUID} ${SUB_SLEEP} ${guid} ${val}`)
        }
...
ipcMain.handle('powerplan:activateByGuid', async (_, guid: string) => {
    await run('powercfg', `/setactive ${guid}`)
    send(`Activated power plan: ${guid}`)
    return true
})
...
ipcMain.handle('powerplan:applyProfile', async (_, profile: string) => {
    ...
    await run('powercfg', `/setacvalueindex ${MA_PLAN_GUID} ${SUB_PROC} be337238-0d82-4146-a960-4f3749d470c7 ${boostModeAc[profile] ?? 2}`)
    await run('powercfg', `/setacvalueindex ${MA_PLAN_GUID} ${SUB_PROC} 0cc5b647-c1df-4637-891a-dec35c318583 ${minCoresAc[profile] ?? 50}`)

    if (profile === 'battery') {
        await run('powercfg', `/setacvalueindex ${MA_PLAN_GUID} ${SUB_SLEEP} 29f6c1db-86da-48c5-9fdb-f2b67b1f44da ${sleepAc[profile]}`)
    } else {
