...
function validatePath(p: any): string {
...
ipcMain.handle('powerplan:export', async (_, savePath: string) => {
    try {
        const validPath = validatePath(savePath)
        await run('powercfg', ['/export', validPath, MA_PLAN_GUID])
        send(`Plan exported to ${validPath}`)
        return true
    } catch (e: any) {
        sendError(`Failed to export power plan: ${e.message}`)
        return false
    }
})
...
ipcMain.handle('powerplan:import', async (_, filePath: string) => {
    try {
        ...
        await run('powercfg', ['/import', validPath])
        send(`Plan imported from ${validPath}`)
        return true
    } catch (e: any) {
        sendError(`Failed to import power plan: ${e.message}`)
        return false
    }
})
